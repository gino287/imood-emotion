...
讀 `_local/out/*.jsonl` 與同名 `.meta.json`，寫出 `baseline/results/summary.md`。
...
DEFAULT_OUT = Path("baseline/results/summary.md")
...
def load_runs(in_dir: Path) -> list:
    ...
    判準是「有沒有配對的 .meta.json」：只有 baseline/run_baseline.py 會同時產出這兩個檔案，
...
def render(stats: list) -> str:
    ...
    lines += [
        ...
        "由 `baseline/checks/summarize.py` 產出，逐句結果不進版控。",
        "",
    ]
    return "\n".join(lines)
...
def main():
    ...
    if not runs:
        raise SystemExit(f"{args.in_dir} 底下沒有結果檔，請先執行 baseline/run_baseline.py")
