...
逐句結果留在本地，摘要寫 baseline/results/fragment_robustness.md（不含資料集原文）。
"""
import argparse
import json
import statistics
...
from emotion.classifier import MODEL_ID, EmotionClassifier, resolve_device  # noqa: E402

DEFAULT_SAMPLES = Path("_local/samples.jsonl")
DEFAULT_OUT = Path("baseline/results/fragment_robustness.md")
# 刻意不放 _local/out/：那裡是 baseline/run_baseline.py 的執行結果，
# 混進去會被 baseline/checks/summarize.py 一起讀走
...
def main():
    ...
    if not args.samples.exists():
        raise SystemExit(
            f"找不到 {args.samples}\n"
            "請先執行：docker compose -f docker/docker-compose.yml run --rm app-cpu python baseline/prepare_samples.py --limit 200"
        )
...
def write_outputs(records, errors, edge_results, device, out_path):
    ...
    lines += ["", "---", "",
              "由 `baseline/checks/stress_fragments.py` 產出。逐句結果含資料集原文，不進版控。", ""]
